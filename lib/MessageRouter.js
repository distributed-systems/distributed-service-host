!function() {
	...
	module.exports = new Class({
		...
		, handleMessage: function(message) {
			...
			if (!message || !message.isDistributedMessage) throw new Error('Cannot route message, message is not a message!');
			...
			// try name based routing
				...
				if (this._routes[recipient.applicationId]) {
					...
					if (this._routes[recipient.applicationId][recipient.id]) {
						...
						else message.notRoutable('Service version «'+recipient.version+'» is not compatible with any of the laoded «'+recipient.applicationId+':'+recipient.id+'» services!');
					}
