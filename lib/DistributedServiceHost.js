!function() {
    ...
    module.exports = new Class({
        ...
        , registerService: function(serviceDefinition) {
            ...
            if (this._serviceRegistry[serviceDefinition.name] && this._serviceRegistry[serviceDefinition.name][serviceDefinition.version]) return Promise.reject(new Errro('Aservice with the same name and version was already installed before!'));
         ...
         */
        , executeService: function(serviceName, semanticServiceVersion, secureToken, applicationId) {
            var comatibleVersion, Service, service;

            if (!type.string(serviceName)) return Promise.reject(new Error('Please specific the name of the service you want to run!'));
            if (!type.string(semanticServiceVersion)) return Promise.reject(new Error('Please specific the semantic version of the service you want to run!'));
            if (!type.string(secureToken)) return Promise.reject(new Error('Please specific the secure token for the service you want to run!'));


            // there cannot be multiple services with the same secure token
            if (this._services[secureToken]) return Promise.reject(new Error('The secureToken exists already. They should be secure, thus a collisoion should never happen!'));
