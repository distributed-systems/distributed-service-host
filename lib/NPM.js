			...
			return new Promise(function(resolve, reject) {
				this.list().then(function(tree) {
					if (tree && tree.dependencies && tree.dependencies[packageName]) {
						...
						else resolve({package: packageName, version: tree.dependencies[packageName].version});
					}
					else resolve();
			...
			return new Promise(function(resolve, reject) {
				...
				this.isInstalled(module, version).then(function(installedPackage) {
					if (installedPackage) resolve(installedPackage);
						...
						// install
						this._queueCommand('install', [[module+(version ? '@'+version : '')]], function(resolve, reject, avalablePackagesList, packageTree, packageString) {
							var match = /([^@]+)@(\S+)/gi.exec(packageString);
							resolve({package: match[1], version: match[2]});
